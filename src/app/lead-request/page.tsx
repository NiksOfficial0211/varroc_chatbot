...
const LeadRequestListing = () => {
  ...
  const downloadExport = async () => {
    ...
    setLoading(true);
    const response = await fetch("/api/export-dealership", {
      method: "POST",
      headers: {
        "Content-Type": "application/json", // 🔥 Important for raw JSON
        "Authorization": `Bearer ${process.env.NEXT_PUBLIC_API_SECRET_TOKEN}`
                    ...
                    {dealershipEnqList && dealershipEnqList.length > 0 &&
                      dealershipEnqList.map((request) => (
                        <div className="row list_listbox" style={{ alignItems: "center", cursor: "pointer" }} key={request.pk_deal_id} onClick={() => { }}>
                          ...
                          <div className="col-lg-2 text-center"><div className="label">    {request.city.charAt(0).toUpperCase() + request.city.slice(1).toLowerCase()}</div></div>
                          <div className="col-lg-2 text-center"><div className="label">    {request.state_address.charAt(0).toUpperCase() + request.state_address.slice(1).toLowerCase()}</div></div>
